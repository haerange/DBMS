...
select ename as 이름, job as 직급, sal 급여 from emp order by job, sal desc;
...
select ename, hiredate, to_char(hiredate, 'YYYY-MM-DD'), deptno
from emp
...
문30) 다음의 SQL문을 분석하시오
      select empno, sal 
      from emp
      where not sal>200 and sal<300    --200 이하면서 300미만인 행 조회
      order by sal;
...
select deptno 부서코드, sum(sal) 급여합계    --실행순서4
from emp                                   --실행순서1
