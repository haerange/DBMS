...
select * from tb_gwamok where gcode like 'd%' and ghakjum=3;   --강사님답


문7)과목테이블에서 프로그램 교과목의 학점 평균보다 낮은 프로그램 교과목을 조회하시오
select gname, round(avg(ghakjum), 0)
from tb_gwamok
group by gname
--having round(avg(ghakjum), 0)
order by round(avg(ghakjum), 0) desc;
